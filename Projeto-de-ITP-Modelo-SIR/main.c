...
typedef struct {
    ...
    float b, k, N_b, T_b, T_b2, S_b, I_b, m_k, n_k, T_k, T_k2, tempoT_b2, tempoT_k2;
} cenarios;
...
void le_arquivo(cenarios *v, FILE *f, float *p){
    char aux[10];
...
int main(int argc,char *argv[]){
    ...
    f_in = fopen(argv[2],"r"); //ABERTURA DO ARQUIVO DE ENTRADA
    ...
    le_arquivo(c, f_in, &periodo); //LEITURA DO ARQUIVO
    ...
    if(*argv[1]=='0' || *argv[1]=='3'){
        ...
        while (c[0].t < periodo) imprime_dados(f_out, c, 0);
    ...
    if(*argv[1]=='1' || *argv[1]=='3'){
        ...
        while (c[1].t < periodo){
            ...
            if(c[1].t > c[1].tempoT_b2) break; //MARCO TEMPORAL DA INTRODUÇÃO DO NOVOS PROTOCOLOS (CENÁRIO 1)
        }

        c[1].T_b = c[1].T_b2; //MUDANÇA DO VALOR T_b DE ACORDO COM NOVOS PROTOCOLOS

        calcula_b_k(c, 1);

        while (c[1].t < periodo) imprime_dados(f_out, c, 1);
    ...
    if(*argv[1]=='2' || *argv[1]=='3'){
        ...
        while (c[2].t < periodo){
            ...
            if(c[2].t > c[2].tempoT_b2) break; //MARCO TEMPORAL DA INTRODUÇÃO DOS NOVOS PROTOCOLOS (CENÁRIO 2)
        }

        c[2].T_k = c[2].T_k2; //MUDANÇA DO VALOR T_k DE ACORDO COM NOVOS PROTOCOLOS

        calcula_b_k(c, 2);

        while (c[2].t < periodo) imprime_dados(f_out, c, 2);
